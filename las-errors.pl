...
# (includes error probability to compute neighbours)
#
...
for (my $t=1; $t<=$finish_time; $t+=1){
	...
	foreach my $u (keys %active_drones){
		if ((scalar keys %{$my_covered_events{$u}}) != 0){
			...
			while ($check_altitude == 1){
				...
				$x = $x/(scalar keys %{$my_covered_events{$u}});
				$y = $y/(scalar keys %{$my_covered_events{$u}});
				$my_position{$u} = [$x, $y];
				my $max_r = 0;
	...
	foreach my $e (@$active_events){
		foreach my $u (keys %active_drones){
			if (exists $my_covered_events{$u}{$e}){
				if (!grep {$_ eq $e} @covered){
					push (@covered, $e);
				}
			}
		}
	}
	my $missing = (scalar @$active_events) - (scalar @covered);
	print "# Holy shit! We missed to cover $missing events!\n";
...
printf "# percentage of uncovered events: %.2f%%\n", $overall_missing*100/((scalar @events)*$finish_time);
printf "# %s\n", '$Id: las-errors.pl 196 2014-03-05 10:35:22Z jim $';
	...
	while ($benefit == 1){
		...
		if (defined $closest_event){
			if (scalar keys %{$my_covered_events{$u}} == 0){
				...
				$x += $ecoords{$closest_event}[0];
				$y += $ecoords{$closest_event}[1];
				$x = $x/((scalar keys %{$my_covered_events{$u}}) + 1);
				$y = $y/((scalar keys %{$my_covered_events{$u}}) + 1);
				$my_position{$u} = [$x, $y];
				my $max_r = 0;
				if (distance($ecoords{$closest_event}[0], $x, $ecoords{$closest_event}[1], $y) > $max_r){
					$max_r = distance($ecoords{$closest_event}[0], $x, $ecoords{$closest_event}[1], $y);
				}
				foreach my $e (keys %{$my_covered_events{$u}}){
					if (distance($ecoords{$e}[0], $x, $ecoords{$e}[1], $y) > $max_r){
						$max_r = distance($ecoords{$e}[0], $x, $ecoords{$e}[1], $y);
